...
RegisterNetEvent('billingmech')
AddEventHandler('billingmech', function()
	if not AmbulanceGUI then
		if PlayerData.job.name == 'mechanik' or PlayerData.job.name == 'mechanik2' or PlayerData.job.name == 'mechanik3' or PlayerData.job.name == 'mechanik4' then
			startAnim()
...
function loadDefaultData()
	ESX.TriggerServerCallback('lscmtablet:getDefaultData', function(result)
		SendNUIMessage({
			type = 'loadDefaultData',
			name = result.firstname .. ' ' .. result.lastname,
			jobgrade = PlayerData.job.label .. ' - ' .. PlayerData.job.grade_label,
			lscmonline = result.mechanic,
...
function loadHistory()
	ESX.TriggerServerCallback('lscmtablet:getHistory', function(result)
		if result ~= nil then		
			local wynik = sortowanie(result)
			for k,v in ipairs(wynik) do
				if k == 256 then break end
				local data = {identifier = v.doctor}
				ESX.TriggerServerCallback('lscmtablet:parseDoctor', function(_doctor)
					SendNUIMessage({
						type = 'loadHistory',
						name = v.name,
						surname = v.surname,
...
function loadUserAssistance(data)
	ESX.TriggerServerCallback('lscmtablet:getUserAssistance', function(result)
		if result ~= false then
			for i = 1, #result, 1 do
				local _data = { identifier = result[i].identifier, digit = result[i].digit }
				ESX.TriggerServerCallback('lscmtablet:getAssistance', function(_assistance)
					SendNUIMessage({
...
RegisterNUICallback('getInvoiceTarget', function()
	...
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		ESX.TriggerServerCallback('lscmtablet:parseName', function(result)
			SendNUIMessage({
...
function createInvoice(target, data)
	...
	ESX.TriggerServerCallback('lscmtablet:addHistory', function()
