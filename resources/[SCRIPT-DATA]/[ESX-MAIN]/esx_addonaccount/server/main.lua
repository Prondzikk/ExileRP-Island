...
MySQL.ready(function()
	...
	local mieszkania = MySQL.Sync.fetchAll('SELECT * FROM properties')
	...
	for i=1, #result, 1 do
		...
		if shared == 0 then
			...
			if #result2 == 0 then
				MySQL.Sync.execute('INSERT INTO addon_account_data (account_name, money, account_money, owner) VALUES (@account_name, @money, @account_money, NULL)',
				{
					['@account_name'] = name,
					['@money']        = 0,
					['@account_money'] = 0
...
AddEventHandler('esx:playerLoaded', function(source)
	...
	for i=1, #AccountsIndex, 1 do
		...
		if account == nil then
			MySQL.Async.execute('INSERT INTO addon_account_data (account_name, money, account_money, owner) VALUES (@account_name, @money, @account_money, @owner)',
			{
				['@account_name'] = name,
				['@money']        = 0,
				['@account_money'] = nil,
