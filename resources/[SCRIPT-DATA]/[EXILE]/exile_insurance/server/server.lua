...
RegisterServerEvent('exile_insurance:delete')
AddEventHandler('exile_insurance:delete', function(identifier, iType)
		...
		function(result)
			...
			for i=1, #result, 1 do
				...
				if aboTime <= nowTime then
					TriggerEvent('exile_insurance:delete', result[i].owner, result[i].type)
				end
			end
		end
	)
