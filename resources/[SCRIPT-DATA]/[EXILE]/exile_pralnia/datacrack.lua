...
function F_02668(arg1)
    Citizen.CreateThread(function()
        ...
        while true do
            F_02539()
            F_02536("Naciśnij ~INPUT_FRONTEND_CANCEL~ aby ~r~przerwać~s~\nNaciśnij ~INPUT_ATTACK~ aby ~g~odblokować")
...
AddEventHandler("datacrack", function(output)
    if output then
       ESX.ShowAdvancedNotification('Lester','~r~Pranie brudnej gotówki','Udało ci się obejść ten ~b~System~s~, moja szkoła!')
       ForceEnd(true)
    else
        ESX.ShowAdvancedNotification('Lester','~r~Pranie brudnej gotówki','~r~Niedobrze~s~, psy dostaną zgłoszenie, ~y~lepiej sie zmywaj')
        ForceEnd(false)
        for i=1, 3, 1 do
