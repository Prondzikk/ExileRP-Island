$(document).ready(function(){
  ...
  document.onkeydown = function (data) {
      ...
      if (data.keyCode  == 69) {
        closeMain()
        $.post('https://exile_taskbarskill/taskEnd', JSON.stringify({taskResult: percent}));
      }
  }
  ...
  //   if (event.keyCode == 69) {
  //     closeMain()
  //     $.post('http://exile_taskbarskill/taskEnd', JSON.stringify({taskResult: percent}));
  //   }
  // })
  ...
  window.addEventListener('message', function(event){
    ...
    if(item.closeFail === true) {
      closeMain()
      $.post('https://exile_taskbarskill/taskCancel', JSON.stringify({tasknum: curTask}));
    }
