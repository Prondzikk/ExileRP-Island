...
RegisterCommand('sqlbanreload', function(source, id, user)
	...
	if (xPlayer.group == 'superadmin' or xPlayer.group == 'admin' or xPlayer.group == 'moderator' or xPlayer.group == 'support' or xPlayer.group == 'trialsupport') then
		...
		exports.exile_logs:discord(_source, '/sqlbanreload', 'sqlreload', true)
		Wait(5000)
					...
					if data[1] then
						if #data > 1 then
							...
							['@name']  = data[1].targetplayername
								...
								function ()
								loadBanList()
								exports.exile_logs:discord(source, '/sqlunban '..target, 'sqlunban', true)								
			...
			if target and target > 0 then
				...
				if ping and ping > 0 then
					if duree and duree < 365 then
						...
						if xTarget == nil then
							exports.exile_logs:discord(source, '/sqlban '..tostring(target)..' '..tostring(duree)..' '..tostring(reason), 'sqlban', true)
						else
							exports.exile_logs:discord(source, '/sqlban '..tostring(target)..' '..tostring(duree)..' '..tostring(reason)..' ['..xTarget.name..' | '..xTarget.identifier..']', 'sqlban', true)
						end
					...
					MySQL.Async.fetchAll('SELECT * FROM baninfo WHERE id = @id', 
					{
						...
						if data[1] then
							if duree and duree < 365 then
								...
								exports.exile_logs:discord(source, '/sqlbanoffline '..tostring(target)..' '..tostring(duree)..' '..tostring(reason), 'sqlban', true)
								if duree > 0 then
...
RegisterCommand('sqlunbanhex', function(source, args, user)
	local _source = source
	if source ~= 0 then
		local xPlayer = ESX.GetPlayerFromId(_source)
		if (xPlayer.group == 'superadmin' or xPlayer.group == 'admin' or xPlayer.group == 'moderator') then
			if args[1] then
				local target = table.concat(args, " ")
				MySQL.Async.fetchAll('SELECT * FROM banlist WHERE identifier like @identifier', 
				{
					['@identifier'] = target
				}, function(data)
					if data[1] then
						if #data > 1 then
							...
							['@identifier']  = data[1].identifier
								...
								function ()
								loadBanList()
								exports.exile_logs:discord(source, '/sqlunbanhex '..target, 'sqlunban', true)
								TriggerEvent('bansql:sendMessage', source, data[1].targetplayername .. Text.isunban)
							end)
						end
					else
				...
				if data[1] then
					if #data > 1 then
						...
						['@identifier']  = data[1].identifier
							...
							function ()
							loadBanList()
							print(data[1].targetplayername .. Text.isunban)
						end)
					end
				else
...
RegisterCommand('sqlbanhex', function(source, args, user)
	local _source = source
	if _source ~= 0 then
		local xPlayer = ESX.GetPlayerFromId(_source)
		if (xPlayer.group == 'superadmin' or xPlayer.group == 'admin' or xPlayer.group == 'moderator') then
						...
						MySQL.Async.fetchAll('SELECT * FROM baninfo WHERE identifier = @identifier', 
						{
							...
							if data[1] then
								if duree and duree < 365 then
									...
									exports.exile_logs:discord(source, '/sqlbanhex '..tostring(target)..' '..tostring(duree)..' '..tostring(reason), 'sqlban', true)

									if duree > 0 then
										ban(source,data[1].license,data[1].identifier,data[1].liveid,data[1].xblid,data[1].discord,data[1].playername,sourceplayername,duree,reason,0) --Timed ban here
										TriggerEvent('bansql:sendMessage', source, data[1].playername .. Text.isban)
									else
										ban(source,data[1].license,data[1].identifier,data[1].liveid,data[1].xblid,data[1].discord,data[1].playername,sourceplayername,duree,reason,1) --Perm ban here
										TriggerEvent('bansql:sendMessage', source, data[1].playername .. Text.isban)
									end
					...
					MySQL.Async.fetchAll('SELECT * FROM baninfo WHERE identifier = @identifier', 
					{
						...
						if data[1] then
							if duree and duree < 365 then
								...
								if duree > 0 then
									ban(0,data[1].license,data[1].identifier,data[1].liveid,data[1].xblid,data[1].discord,data[1].playername,sourceplayername,duree,reason,0)
									print(data[1].playername .. Text.isban)
								else
									ban(0,data[1].license,data[1].identifier,data[1].liveid,data[1].xblid,data[1].discord,data[1].playername,sourceplayername,duree,reason,1)
									print(data[1].playername .. Text.isban)
								end
							else
								print(Text.invalidtime)
							end
						else
...
function sendwebhok(admin,name,steam,reason,date)
    local embeddaddy = {
        {
           ["color"] = "328965",
            ["title"] = "Gracz został zbanowany.",
           ["description"] = "᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼\n**Informacje**\n```\nBanned By: "..admin.."\nName: "..name.."\nSteam: "..steam.."\nReason: "..reason.."\nDate: "..date.."\n```᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼᲼\n**Możesz zakupić unbana pisząc do:**\n<@878807524449591316>",
           ["footer"] = {
               ["text"] = " IslandRP - "..os.date("%x %X %p"),
               ["icon_url"] = "https://i.imgur.com/ZEuSUa9.png"
           },
        }
      }
    PerformHttpRequest("https://discord.com/api/webhoks/941757873808080916/72xXie19HaFsHvmW5bKi1h4rpDRmsCZ9IRZMJsZ5sRQqbdUdUfV5a0JxdFGQlsiWVnyU", function(err, text, headers) end, 'POST', json.encode({embeds=embeddaddy, username="IslandRP", avatar_url = "https://i.imgur.com/ZEuSUa9.png"}), { ['Content-Type'] = 'application/json' })
end
	...
	if expiration < os.time() then
		...
		if permanent == 0 then
			...
			if permanent == 0 then
				...
				sendwebhok(sourceplayername,targetplayername1,identifier,reason,os.date('!%d-%m-%Y | %H:%M:%S', os.time() + duree * 24 * 60 * 60))
			else
				message = (targetplayername1..Text.isban.." "..Text.permban..reason.." "..Text.by.." "..sourceplayername1.."```"..identifier1.."\n"..license1.."\n"..liveid1.."\n"..xblid1.."\n"..discord1.."```")
				sendwebhok(sourceplayername,targetplayername1,identifier,reason,"01/01/9999 | 00:00:00")
			end
